...
export interface PdfOpts {
	...
	filepath: string;
	callback: () => void;
	hooks?: { before_script?: (doc: any) => void };
}

function createSimpleStream(filepath: string): any {
	const simplestream: any = {
		chunks: [] as Buffer[],
		filepath,
	};
	simplestream.on = function (_event: string, callback: () => void) {
		this.callback = callback;
	};
	simplestream.once = function () {};
	simplestream.emit = function () {};
	simplestream.write = function (chunk: Buffer) {
...
function initDoc(opts: PdfOpts): any {
	const print = opts.print;
	const fonts = opts.config.fonts || null;
	const options = {
		compress: false,
	...
	const doc: any = new PDFDocument(options);
	...
	const inner_text = doc.text.bind(doc);

	doc.simple_text = function (...args: any[]) {
		doc.font('ScriptNormal');
		inner_text(...args);
	};

	doc.format_text = function (text: string, x: number, y: number, options?: any) {
		const cache_current_state = doc.format_state;
		doc.reset_format();
	...
	doc.text = function (text: string, x: number, y: number, options?: any) {
		options = options || {};
		let color: string = options.color || 'black';
		...
		const split_for_formatting: string[] = text
			.split(/(\\\*)|(\*{1,3})|(\\?_)|(\[\[)|(\]\])/g)
			.filter((a: string | undefined): a is string => !!a);

		const font_width = print.font_width;
		for (const elem of split_for_formatting) {
			if (elem === '***') {
...
function finishDoc(doc: any, callback: () => void, filepath: string): void {
	const stream = doc.pipe(createSimpleStream(filepath));
	doc.end();
...
function generate(doc: any, opts: PdfOpts): void {
